...
class EngineBridge:
    ...
    def _build_preflop_analysis_text(self, decision: HeroDecision, hand=None) -> str:
        ...
        lines.append(f"Tree / node_type: {debug.get('node_type', '-')}")
        lines.append(f"Range owner: {debug.get('range_owner', '-')}")
        lines.append(f"Hero hand class: {pre.hand_class}")
        description = pre.meta.get("description") if isinstance(pre.meta, dict) else None
        if description:
        ...
        lines.append("")
        lines.append("Note: preflop layer is chart/tree based. EV branches are not calculated here; the file shows the exact node and chart branch used.")
        return "\n".join(lines)
