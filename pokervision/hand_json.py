...
def _canonical_hand_json_name(hand_id: str) -> str:
    return f"hand_{_hand_number_suffix(hand_id)}.json"
...
class JsonStore:
    ...
    def _resolve_hand(self, hero_cards: list[str]) -> HandMatch:
        ...
        if matching_meta is not None:
            ...
            canonical_hand_path = hand_dir / _canonical_hand_json_name(hand_id)
            legacy_hand_path = Path(str(matching_meta.get("hand_json_path") or hand_dir / "hand.json"))
            hand_path = canonical_hand_path
            hand_payload = self._read_json(canonical_hand_path, default={})
            if not hand_payload and legacy_hand_path != canonical_hand_path:
                hand_payload = self._read_json(legacy_hand_path, default={})
            if not hand_payload:
        ...
        hand_path = hand_dir / _canonical_hand_json_name(hand_id)
        hand_payload = self._new_hand_payload(hand_id=hand_id, hero_cards=hero_cards)
        return HandMatch(
            hand_id=hand_id,
            hand_path=hand_path,
    ...
    def _load_index(self) -> dict[str, Any]:
        ...
        if self.hands_dir.exists():
            ...
            for pattern in ("hand_*/hand_*.json", "hand_*/hand.json"):
                for candidate in sorted(self.hands_dir.glob(pattern)):
                    if candidate not in discovered_paths:
                        discovered_paths.append(candidate)
            for hand_json_path in discovered_paths:
                payload = self._read_json(hand_json_path, default={})
                if not isinstance(payload, dict):
                    continue
                hand_id = str(payload.get("hand_id") or hand_json_path.parent.name)
                canonical_path = hand_json_path.parent / _canonical_hand_json_name(hand_id)
                hands.append(
                    {
                        "hand_id": hand_id,
                        ...
                        "hand_json_path": str(canonical_path),
                    }
                )
