...
class PipelineSolverBridgeHookTests(unittest.TestCase):
    def test_pipeline_calls_solver_bridge_after_hand_update_and_before_render_state(self):
        ...
        solver_idx = source.index("solver_preview = build_solver_contract_preview(analysis, hand, self.settings)")
    ...
    def test_render_state_uses_only_top_level_solver_contract(self):
        source = Path(__file__).resolve().parents[1].joinpath("render_state.py").read_text(encoding="utf-8")
        self.assertIn("_clear_legacy_solver_annotation", source)
        self.assertIn('cleaned.pop("solver_bridge", None)', source)
        self.assertNotIn('"solver_bridge": solver_summary', source)
        self.assertIn("advisor_input=dict(hand.advisor_input)", source)
        self.assertIn("solver_input=dict(hand.solver_input)", source)
        self.assertIn("solver_output=dict(hand.solver_output)", source)
